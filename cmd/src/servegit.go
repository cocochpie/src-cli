...
USAGE
  src [-v] serve-git [-list] [-bare] [-addr :3434] [path/to/dir]

By default 'src serve-git' will recursively serve your current directory on the address ':3434'.

'src serve-git -bare' will enable bare repository serving. It will create a '/.git' symbolic links in each bare repository.
useful for serving mirror-packed repositories.
...
`)
	...
	var (
		...
		bareFlag = flagSet.Bool("bare", false, "enable bare repository serving")
	)
